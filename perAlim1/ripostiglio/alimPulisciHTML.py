...
#sgn b=open(newFile,'w')
##with open(oldFile) as f:
##    #testoBR = ' '.join(line.strip() for line in f).replace('<BR> ','<BR>\n')
##    testoFuso = ' '.join(line.strip() for line in f)    # è una stringa
##    testoBR = testoFuso.split('<BR> ')    # è una lista
#sgn b.close()

#testoFuso = ' '.join(line.strip('\n') for line in testoPulito)    # è una stringa
#testoFuso = ' '.join(line.strip('\n') for line in testoPulito).replace('<BR> ','<BR>\n<§§§>') # stringa
#testoBR = testoFuso.split('\n')    # è una lista


testoBR = ' '.join(line.strip('\n') for line in testoPulito).replace('<BR> ','<BR>\n<fineriga>').split('<fineriga>')    # è una lista

# debug: scrivi Testo BR in un file
with open('testoBR.txt','w') as d:
    for line in testoBR:
...
for rigaNuova in testoBR:
        ...
        if rigaVecchia is not '':   # Se non siamo alla prima riga, quindi c'è una riga precedente
                ...
                testoTrattini.append(rigaVecchia)
